			...
			for i,v in ipairs(Particle.array) do
				if v.live --[[and checkolor(w.c,v.c)--]] then
					if bound_point_circle(v.x, v.y, w.x, w.y, w.r) then
						v.tempr = v.r
						v.r = ((v.r * math.sin(v.phi) + w.a * (1 - (((v.x - w.x)^2+(v.y - w.y)^2)^(1/2))/w.r) * 75 * dt * math.sin(w.phi))^2 + (v.r * math.cos(v.phi) + w.a *(1 - (((v.x - w.x)^2+(v.y - w.y)^2)^(1/2))/w.r) * 75 *dt * math.cos(w.phi))^2 )^(0.5)
...
function love.load()
	...
	Redirector:new(2*width/3,height/2,math.rad(120),25,0.03)
...
function love.draw()
	...
	love.graphics.push()
	for i, v in ipairs(Particle.array) do
		love.graphics.print(v.phi, 100, i*100)
	end
	Redirector:draw()
	Emitter:draw()
	Sink:draw()
